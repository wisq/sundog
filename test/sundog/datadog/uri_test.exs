defmodule Sundog.Datadog.UriTest do
  use ExUnit.Case
  ...
  test "datadog_uri/2 falls back to get_os_hostname/0" do
    ...
    assert %URI{query: query} = DDURI.datadog_uri("/path3")

    q = URI.decode_query(query)
    assert q["host"] == DDURI.get_os_hostname
  end

  test "get_os_hostname/0 is memoized" do
    host1 = DDURI.get_os_hostname
    with_mock System, [cmd: fn(_, _) -> raise "System.cmd called" end] do
      assert DDURI.get_os_hostname == host1
    end
  end
end
